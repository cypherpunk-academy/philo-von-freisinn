#!/usr/bin/env python3
"""
Doppelmatrix auf DIN A0 — A2-Inhalt, Freiraum unter beiden Tafeln ausgeglichen.
...
A2 kommt aus build_a2() in doppelmatrix-halbkreise.py (kein SVG-Parse).
...
YAML_PATH = os.path.join(_DIR, "doppelmatrix-gesund-und-krank.yaml")
HINTERGRUND = "doppelmatrix-hintergrund.jpg"
OUT_SVG = os.path.join(_DIR, "doppelmatrix-halbkreise-a0.svg")
...
DPI = 300
OUT_JPG = os.path.join(_DIR, "doppelmatrix-halbkreise-a0.jpg")
...
def _a2_module():
    path = os.path.join(_DIR, "doppelmatrix-halbkreise.py")
    spec = importlib.util.spec_from_file_location("doppelmatrix_halbkreise", path)
...
def main():
    ...
    hg = os.path.join(_DIR, HINTERGRUND)
    ...
    png_tmp = os.path.join(_DIR, ".doppelmatrix-a0-render.png")
    try:
        subprocess.run(
